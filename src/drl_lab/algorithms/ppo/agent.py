...
from drl_lab.algorithms.vpg.losses import value_loss
...
class PPOAgent:
    ...
    def act(self, obs: NDArray[np.float32]) -> tuple[int, float, float]:
        """Sample an action and return action, old log prob, and value estimate."""
    ...
    def update(
        ...
        last_policy_loss = torch.zeros((), dtype=returns.dtype, device=returns.device)
        last_kl = torch.zeros((), dtype=returns.dtype, device=returns.device)
        last_entropy = torch.zeros((), dtype=returns.dtype, device=returns.device)
        policy_updates = 0
        ...
        for _ in range(self.config.policy_train_iters):
            ...
            entropy = entropy_bonus(self.policy, obs)
            total_policy_loss = loss - self.config.entropy_coef * entropy

            self.policy_optimizer.zero_grad(set_to_none=True)
            torch.autograd.backward(total_policy_loss)
            self.policy_optimizer.step()

            with torch.inference_mode():
                new_log_probs = self.policy.log_prob(obs, actions)
                kl = approx_kl(old_log_probs, new_log_probs)
            last_policy_loss = loss.detach()
            last_entropy = entropy.detach()
            last_kl = kl.detach()
            policy_updates += 1
            if float(kl) > 1.5 * self.config.target_kl:
                break

        last_value_loss = torch.zeros((), dtype=returns.dtype, device=returns.device)
        for _ in range(self.config.value_train_iters):
            loss_v = value_loss(self.value_function, obs, returns)
            self.value_optimizer.zero_grad(set_to_none=True)
            torch.autograd.backward(loss_v)
            self.value_optimizer.step()
            last_value_loss = loss_v.detach()
        ...
        return {
            ...
            "policy_updates": float(policy_updates),
        }
