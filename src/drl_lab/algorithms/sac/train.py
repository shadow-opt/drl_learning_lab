...
def train(config: SACConfig) -> dict[str, float]:
    ...
    if not np.allclose(action_high, -action_low):
        raise ValueError("SAC actor assumes symmetric action bounds")
    action_limit = float(action_high.max())

    agent = SACAgent(obs_dim, action_dim, action_limit, config, device)
    ...
    last_temperature_loss = 0.0
    last_alpha = float(agent.alpha.detach())
    ...
    with CsvLogger(
        ...
        for step in range(config.total_steps):
            ...
            if step < config.learning_starts:
                ...
                action = agent.act(obs_array, deterministic=False)
            ...
            if (
                ...
                last_temperature_loss = update_metrics["temperature_loss"]
                last_alpha = update_metrics["alpha"]
                ...
                logger.log(
                    {
                        ...
                        "temperature_loss": last_temperature_loss,
                        "alpha": last_alpha,
    ...
    save_checkpoint(
        ...
        extra={"last_eval_return": last_eval_return, "alpha": last_alpha},
    )
    save_checkpoint(
    ...
    return {
        ...
        "last_temperature_loss": last_temperature_loss,
...
def parse_args() -> argparse.Namespace:
    ...
    parser.add_argument("--learning-starts", type=int, default=SACConfig.learning_starts)
    ...
    parser.add_argument("--run-dir", type=Path, default=SACConfig.run_dir)
