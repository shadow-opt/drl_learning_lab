...
from drl_lab.algorithms.td3.eval import evaluate
...
def train(config: TD3Config) -> dict[str, float]:
    ...
    if not np.allclose(action_high, -action_low):
        raise ValueError("TD3 actor assumes symmetric action bounds")
    action_limit = float(action_high.max())

    agent = TD3Agent(obs_dim, action_dim, action_limit, config, device)
    ...
    critics_output_shapes = [
        list(output.shape) for output in agent.critics(example_input, example_action)
    ]
    save_export_report(
        config.run_dir,
        {
            ...
            "critics": {
                "path": critics_onnx_path,
                "input_names": ["obs", "actions"],
                "output_names": ["q1", "q2"],
                "input_shape": [list(example_input.shape), list(example_action.shape)],
                "output_shape": critics_output_shapes,
                "max_abs_diff": critics_result.max_abs_diff,
                "mean_abs_diff": critics_result.mean_abs_diff,
                "passed": critics_result.passed,
            },
        },
    )
...
def parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(description="Train TD3 on Pendulum.")
    parser.add_argument("--total-steps", type=int, default=TD3Config.total_steps)
    parser.add_argument("--learning-starts", type=int, default=TD3Config.learning_starts)
    parser.add_argument("--eval-episodes", type=int, default=TD3Config.eval_episodes)
    parser.add_argument("--seed", type=int, default=TD3Config.seed)
    parser.add_argument("--run-dir", type=Path, default=TD3Config.run_dir)
