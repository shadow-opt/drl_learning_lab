...
- Demo：`curriculum/chapters/08_export_deployment/code/export_demo.py`
...
```bash
conda run -n drl-lab python curriculum/chapters/08_export_deployment/code/export_demo.py
```
...
然后选择一个算法训练入口，例如 DQN 或 SAC，检查 run 目录里的 ONNX artifact。在 `curriculum/chapters/08_export_deployment/report.md` 记录：
