...
- 教学 demo：`curriculum/chapters/04_dqn/code/dqn_smoke_demo.py`
...
```bash
conda run -n drl-lab python curriculum/chapters/04_dqn/code/dqn_smoke_demo.py
```
...
然后完成 `curriculum/chapters/04_dqn/exercises.md`，在 `curriculum/chapters/04_dqn/report.md` 写出 DQN target 和一次失败记录。
